...
const ItemSelected = () => {
    ...
    // State variables to manage name, quantity, and action
    const [name, setName] = useState('');
    const [quantity, setQuantity] = useState('');
    const [action, setAction] = useState('');

    // Function to handle the submission of the selected item and action
    const submitEvent = () => {
        const item = {
            name: name,
        ...
        // Dispatching an action based on the selected action (Add or Reduce)
        if (action === "Reduce") {
            dispatch({
                type: 'RED_QUANTITY',
                payload: item,
